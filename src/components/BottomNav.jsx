...
// 앱 하단 공통 내비게이션 바
const NAV_ITEMS = [
  { path: "/home", label: "홈", icon: "/icons/gnb/home.svg", activeIcon: "/icons/gnb/home-active.svg" },
  {
    path: "/my-school",
    label: "내 학교",
    icon: "/icons/gnb/my-school.svg",
    activeIcon: "/icons/gnb/my-school-active.svg",
  },
...
function BottomNav() {
  return (
    <nav className={`flex shrink-0 items-center justify-between bg-white px-5 pt-3 ${SAFE_AREA_BOTTOM}`}>
      {NAV_ITEMS.map((item) => (
        <NavLink key={item.path} to={item.path} className="flex flex-col items-center gap-1">
          {({ isActive }) => (
            <>
              <img src={isActive ? (item.activeIcon ?? item.icon) : item.icon} alt="" className="size-8" />
